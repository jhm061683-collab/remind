-- 국어·영어 공통 지문 분리 저장 (추가 AI 비용 없음)
alter table public.questions
  add column if not exists shared_passage text;

comment on column public.questions.shared_passage is
  '공통 지문(국어·영어). problem_latex에는 합본이 들어갈 수 있음.';

-- 기존 save_question_batch에 shared_passage만 추가
