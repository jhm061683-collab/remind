...
export function NotificationComposer({ students }: Props) {
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
      ...
      />
      <div className="max-h-56 overflow-y-auto rounded-xl border border-zinc-200 p-2">
        ...
        <div className="space-y-1">
          {filtered.map((student) => (
            <label key={student.id} className="flex items-center gap-2 rounded-lg px-2 py-1 hover:bg-zinc-50">
              <input
                type="checkbox"
              ...
              <span className="text-sm text-zinc-700">
                {student.displayName} <span className="text-zinc-400">({student.username})</span>
              </span>
            </label>
          ))}
        </div>
      ...
      {message ? <p className="text-xs text-zinc-600 whitespace-pre-line">{message}</p> : null}
    </div>
  );
}
